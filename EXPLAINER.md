...
There may exist a delay (unintentional or malicious) between the time the popup is created during the original [user activation](https://html.spec.whatwg.org/multipage/interaction.html#tracking-user-activation) and the time that the [document.documentElement](https://dom.spec.whatwg.org/#ref-for-dom-document-documentelement%E2%91%A0) becomes available. For example a slow network or intentional throttling by a malicious server may cause a significant delay which would make the popup become fullscreen long after user input. In such case, the user agent would display an empty or loading page until the [document.documentElement](https://dom.spec.whatwg.org/#ref-for-dom-document-documentelement%E2%91%A0) is available and becomes fullscreen. Therefore a web application has no control over the content shown in the popup between the user gesture and the time which the document element is available and made fullscreen.
